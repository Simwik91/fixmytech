...
// Helper functions declared at module level
function updateEanInputFeedback() {
    ...
    if (config.length) {
        feedback = window.i18n.translate('eangenerator_feedback_digits', { type: selectedType.toUpperCase(), length: config.length });
    } else if (config.min && config.max) {
        feedback = window.i18n.translate('eangenerator_feedback_range', { type: selectedType.toUpperCase(), min: config.min, max: config.max });
    } else {
        feedback = window.i18n.translate('eangenerator_feedback_format', { type: selectedType.toUpperCase(), regex: config.regex });
    }
...
async function generateBarcode({ canvas, code, type, errorElement, successElement, downloadButton, svgDownloadButton, loadingElement }) {
  ...
  try {
    ...
    errorElement.textContent = window.i18n.translate('eangenerator_error_invalid_value');
    errorElement.style.display = 'block';
  } finally {
    loadingElement.style.display = 'none';
  }
...
function initializeBarcodeGenerator() {
  ...
  // Listen for language changes to update feedback
  window.addEventListener('langChange', updateEanInputFeedback);
}
...
function validateInput(code, type) {
  ...
  if (!config.regex.test(code)) {
      return window.i18n.translate('eangenerator_error_invalid_format', { type: type, regex: config.regex });
  }
  
  if (config.length && code.length !== config.length) {
      return window.i18n.translate('eangenerator_error_invalid_length', { type: type, length: config.length });
  }

  if (type === 'pharmacode') {
    const num = parseInt(code);
    if (isNaN(num) || num < config.min || num > config.max) {
      return window.i18n.translate('eangenerator_error_invalid_pharmacode_value', { min: config.min, max: config.max });
    }
  }
...
// Global entry point
window.initBarcodeGenerator = function() {
